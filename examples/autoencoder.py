...
class Model(nn.Module):
    def __init__(
        self, vocab_size=10, emb_size=128, hidden_size=128, num_layers=1, use_cuda=False
    ):
        super().__init__()
        ...
        self.features = nn.Sequential(
            nn.Conv1d(emb_size, 32, 3), nn.ReLU(True), nn.Conv1d(32, hidden_size, 3)
        )
...
corpus = ["x*{}+{}".format(i, j) for i, j in product(range(10), range(10))]
...
for i in range(epochs):
    for j in range(0, len(I), batch_size):
        ...
        if i % 10 == 0:
            print(
                "Epoch : {:05d} Avg loss : {:.6f} Avg Precision : {:.6f}".format(
                    i, avg_loss, avg_precision
                )
            )
            print("Generated :")
            model.given(inp)
